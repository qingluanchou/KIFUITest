...
+ (void)showNoticeMsgWithoutImage:(NSString *)message {
...
+ (void)showSync {
    [[self sharedView] showSyncProgress:-1 status:nil maskType:SVProgressHUDMaskTypeNone];
}

-(void)showSyncProgress:(CGFloat)progress status:(NSString*)string maskType:(SVProgressHUDMaskType)hudMaskType {
...
- (NSTimeInterval)displayDurationForString:(NSString*)string {
    NSTimeInterval gap = MIN((float)[self asciiLengthWithString:string]*0.06 + 0.3, 5.0);
    return MAX(gap, 1.0);
}
...
//适配中文
- (NSInteger)asciiLengthWithString:(NSString *)string {
